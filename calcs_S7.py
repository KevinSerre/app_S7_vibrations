...
COMFORT_LEVELS_VERTICAL = {"Maximum": 0.5 , "Mean": 1.0, "Minimum": 2.5}
COMFORT_LEVELS_LATERAL = {"Maximum": 0.1 , "Mean": 0.3, "Minimum": 0.8}
...
class PedestrianBridge:
    ...
    G: float                
    J: float                # 
    mmi: float              # mass moment of inertia
    
    #@property
    #def mass_occupied(self):
     #   return self.mass + mass_pedestrians(d, self.L, self.B)
...
def critical_vertical_frequencies(
    ...
    CRITICAL_RANGE_START = 1    # Hz
    CRITICAL_RANGE_END = 13     # Hz
    critical_freqs = [freq for freq in vertical_freqs if CRITICAL_RANGE_START <= freq[1] <= CRITICAL_RANGE_END]
...
def critical_lateral_frequencies(
    ...
    as per CSA S7 Cl.7.3.2.3.
    """     
    CRITICAL_RANGE_START = 0.3  # Hz
    CRITICAL_RANGE_END = 6.5    # Hz
    critical_freqs = [freq for freq in lateral_frequencies if CRITICAL_RANGE_START <= freq[1] <= CRITICAL_RANGE_END]
    return critical_freqs
